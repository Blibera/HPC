...
f = open("C:/Users/Slayer/Desktop/공유할것/NPB_log_knl02_64/NPB_EP_log/pidstat_tswch/pidstat_tswch_2018-08-02.txt", 'r')
f_csv = open("C:/Users/Slayer/Desktop/공유할것/NPB_log_knl02_64/NPB_EP_log/pidstat_tswch/pidstat_tswch_2018-08-02.csv", 'w',
             newline='')
# 변수
stack = 0
string = ""
...
count = string.count('Average:      UID       PID   cswch/s nvcswch/s  Command')
print(count)
for i in range(0, count):
    time = string.split('Average:      UID       PID   cswch/s nvcswch/s  Command')[i]
...
for i in range(0, count):
    ...
    time = time.split('PM')[0]
    time = time.strip()
    split_line = string.split('Average:      UID       PID  minflt/s  majflt/s     VSZ    RSS   %MEM  Command')[i]
    print(time)
    avg_count = split_line.count('Average:')
    line_comma = split(split_line)
    print(avg_count)
